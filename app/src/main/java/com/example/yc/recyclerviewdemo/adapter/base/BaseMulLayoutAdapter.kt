 ...
 */
abstract class BaseMulLayoutAdapter<T>(var mDatas: MutableList<T>, var mContext: Context) : RecyclerView.Adapter<BaseMulLayoutAdapter<T>.BaseViewHolder>() {
    ...
    override fun onCreateViewHolder(parent: ViewGroup, viewType: Int):BaseViewHolder {
        val typeView = getLayoutId()
        if (typeView.size() == 0)
            throw Exception("请返回有效的布局id")

        val view = LayoutInflater.from(mContext).inflate(typeView[viewType], parent, false)
    ...
    override fun getItemViewType(position: Int): Int {
        return getType(mDatas, position)
    }
     ...
     * 获取ViewType
     */
    abstract fun getType(mDatas: MutableList<T>, position: Int): Int

    /**
     * 抽象方法，返回每种viewType对应的ViewId
     */
    abstract fun getLayoutId(): SparseArray<Int>
